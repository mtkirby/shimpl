...
if ( $dhcp{'dodhcp'} == 1 ) {
    ...
    print FD
qq(option rfc3442-classless-static-routes code 121 = array of unsigned integer 8;\n);
    if ($shimhostname) {
...
# shim-dhclient-script is modified to output 'set' and we will grab the vals below
        @dhclientout =
`ip netns exec $shimns dhclient -sf /root/shim-dhclient-script -cf /tmp/dhclient-${shimns}.conf $shimif`;
    ...
    &runcmd( 1,
        "ip netns exec $shimns ip addr add dev $shimif $shimip/$shimcidr" );
}
...
if ( !$routermac ) {
    ...
    &runcmd( 1,
        "ip netns exec $shimns arp -i $shimif -s $routerip $routermac" );
}
...
if ( $? != 0 ) {
    ...
    &runcmd( 1,
        "ip netns exec $routerns ip link set dev tap1-$pid name $routerif" );
    &runcmd( 1, "ip netns exec $routerns ip link set dev $routerif up" );
    &runcmd( 1,
        "ip netns exec $routerns ip link set dev $routerif address $routermac"
    );

    if ($vlan) {
        &runcmd( 1,
            "ip link add link $routerif name $routerif type vlan id $vlan" );
        $routerif .= ".$vlan";
...
"ip netns exec $routerns ip link set dev $routerif address $routermac"
    ...
    &runcmd( 1,
"ip netns exec $routerns ip address add dev $routerif ${routerip}/${shimcidr}"
    );
...
&runcmd( 1,
    "ip netns exec $routerns ip address add dev tap2 ${routerbrip}/16" );
...
&runcmd( 1,
    "ip netns exec $routerns ip rule add from $shimip lookup $shimroutetable" );
&runcmd( 1,
"ip netns exec $routerns ip route add default via $shimbrip table $shimroutetable"
);

if ($carveports) {
    &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I PREROUTING -d $shimip -p tcp -m tcp -m multiport --dports $carveports -j DNAT --to-destination 169.254.0.1"
    );
    &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I PREROUTING -d $shimip -p udp -m udp -m multiport --dports $carveports -j DNAT --to-destination 169.254.0.1"
    );
    &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p tcp -m tcp -m multiport --dports $carveports -j SNAT --to-source $shimbrip"
    );
    &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p udp -m udp -m multiport --dports $carveports -j SNAT --to-source $shimbrip"
    );

    &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I PREROUTING -s $shimip -d $routerip -p tcp -m tcp -m multiport --dports $carveports -j DNAT --to-destination 169.254.0.1"
    );
    &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I PREROUTING -s $shimip -d $routerip -p udp -m udp -m multiport --dports $carveports -j DNAT --to-destination 169.254.0.1"
    );
    &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p tcp -m tcp -m multiport --dports $carveports -j SNAT --to-source $routerbrip"
    );
    &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p udp -m udp -m multiport --dports $carveports -j SNAT --to-source $routerbrip"
    );
}
...
if (@rdrports) {
    foreach ( keys %rdrport ) {
        &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I PREROUTING -d $shimip -p tcp -m tcp --dport $_ -j DNAT --to-destination 169.254.0.1:$rdrport{$_}"
        );
        &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I PREROUTING -d $shimip -p udp -m udp --dport $_ -j DNAT --to-destination 169.254.0.1:$rdrport{$_}"
        );
        &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p tcp -m tcp --dport $rdrport{$_} -j SNAT --to-source $shimbrip"
        );
        &runcmd( 1,
"ip netns exec $shimns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p udp -m udp --dport $rdrport{$_} -j SNAT --to-source $shimbrip"
        );

        &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I PREROUTING -s $shimip -p tcp -m tcp --dport $_ -j DNAT --to-destination 169.254.0.1:$rdrport{$_}"
        );
        &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I PREROUTING -s $shimip -p udp -m udp --dport $_ -j DNAT --to-destination 169.254.0.1:$rdrport{$_}"
        );
        &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p tcp -m tcp --dport $rdrport{$_} -j SNAT --to-source $routerbrip"
        );
        &runcmd( 1,
"ip netns exec $routerns iptables -t nat -I POSTROUTING -d 169.254.0.1 -o tap2 -p udp -m udp --dport $rdrport{$_} -j SNAT --to-source $routerbrip"
        );
    }
}

&runcmd( 1,
"ip netns exec $shimns iptables -t nat -A PREROUTING -d $shimip ! -p icmp -j DNAT --to-destination $routerbrip"
);
&runcmd( 1,
"ip netns exec $shimns iptables -t nat -A PREROUTING -d $shimbrip ! -p icmp -j DNAT --to-destination $routerip"
);
&runcmd( 1,
"ip netns exec $shimns iptables -t nat -A POSTROUTING -o $shimif ! -d 169.254.0.0/16 -j SNAT --to $shimip"
);

&runcmd( 1,
"ip netns exec $routerns iptables -t nat -A PREROUTING -d $routerip ! -p icmp -j DNAT --to-destination $shimbrip"
);
&runcmd( 1,
"ip netns exec $routerns iptables -t nat -A PREROUTING -d $routerbrip ! -p icmp -j DNAT --to-destination $shimip"
);
&runcmd( 1,
"ip netns exec $routerns iptables -t nat -A POSTROUTING -s $shimip -j SNAT --to $routerbrip"
);

# hide from traceroutes
&runcmd( 0,
"ip netns exec $routerns iptables -t mangle -D PREROUTING -m ttl --ttl-gt 1  -j TTL --ttl-inc 2"
);
&runcmd( 1,
"ip netns exec $routerns iptables -t mangle -A PREROUTING -m ttl --ttl-gt 1  -j TTL --ttl-inc 2"
);

# prevent dhcp clients from detecting a duplicate ip
&runcmd( 1,
    "ip netns exec $routerns arptables -I INPUT -s $shimip -d $shimip -j DROP"
);
...
&runcmd( 1,
"ip netns exec $routerns arptables -A INPUT -i $routerif -s $shimip -j ACCEPT"
);
&runcmd( 1,
"ip netns exec $routerns arptables -A INPUT -i $routerif --source-mac $shimmac -j ACCEPT"
);
...
&runcmd( 1,
    "ip netns exec $shimns arptables -A INPUT -i $shimif -d $shimip -j ACCEPT"
);
&runcmd( 1,
"ip netns exec $shimns arptables -A INPUT -i $shimif --destination-mac $shimmac -j ACCEPT"
);
...
if ( $dhcp{'dodhcp'} == 1 ) {
    ...
    if ( $dhcp{'addhost'} == 1 ) {
        ...
        if ( $dhcp{'new_domain_name_servers'} ) {
            print FD qq(	option domain-name-servers )
              . join( ', ', @{ $dhcp{'new_domain_name_servers'} } ) . qq(;\n);
        }
        if ( $dhcp{'new_ntp_servers'} ) {
            print FD qq(	option ntp-servers )
              . join( ', ', @{ $dhcp{'new_ntp_servers'} } ) . qq(;\n);
        }
        if ( $dhcp{'new_netbios_name_servers'} ) {
            print FD qq(	option netbios-name-servers )
              . join( ', ', @{ $dhcp{'new_netbios_name_servers'} } ) . qq(;\n);
        }
        if ( $dhcp{'new_netbios_node_type'} ) {
            print FD
qq(	option netbios-node-type ${dhcp{'new_netbios_node_type'}[0]};\n);
        }
        ...
        &runcmd( 1,
"ip netns exec $routerns /usr/sbin/dhcpd -q -lf /var/lib/dhcp/dhcpd-${routerns}.leases -cf /tmp/shimdhcpd-${routerns}.conf -pf /var/run/dhcpd-${routerns}.pid $routerif"
        );
    }
...
&runcmd( 1,
    "ip netns exec $routerns sysctl -w net.ipv4.conf.tap1.proxy_arp=1" );
